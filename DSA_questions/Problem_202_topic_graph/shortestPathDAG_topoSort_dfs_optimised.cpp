...
class Solution {
  public:
    void topoSort(int node,vector<vector<pair<int,int>>>& adj,vector<int>& topoList,vector<int>& visited){
        visited[node] = 1;// mark as visited
        for(auto nei : adj[node]){
            auto [neiNode,neiWeight] = nei;
            // need only Node in this case ignore weight -- we know it's a DAG so no need to check for cycle or not
            if(visited[neiNode]==0) topoSort(neiNode,adj,topoList,visited);
        }
        topoList.push_back(node);
    }
    vector<int> shortestPath(int V, int E, vector<vector<int>>& edges) {
        // method 2 : topo sort and then relax each branch once -- topo sort with dfs - shortestPathDAG_topoSort_dfs
        ...
        topoSort(0,adj,topoList,visited);
